...
const validateRequest = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!errors.isEmpty()) {
    return next(ErrorFactory.createError(ErrorTypes.BadRequest, 'Errore di validazione: ' + errors.array().map(e => e.msg).join(', '))); // mappo ed unisco tutti i messaggi di errore in un array
  }
